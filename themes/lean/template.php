...
function _lean_make_day_count($datefield, $link = null) {
 ...
 * Remove tab
 */
function _lean_removetab($label, &$vars) {
  $tabs = explode("\n", $vars['tabs']);
  $vars['tabs'] = '';
 ...
 */
function _lean_tag_cloud($result, $max_count=10) {
  $terms_output = '';
 ...
 */
function _lean_get_today_title($node, $orig_title, $link_to_node=false, $show_as_today=false, $output_as_dl=false) {

  // Get prefix for today's title eg. 'Today', 'Yesterday'..
  if (!$show_as_today) {
    $prefix = _lean_output_ago_date($node->created);
  } else {
...
function _lean_get_section_name($node) {
  if (isset($node->node_section)) { //$node->node_section is set here in lean_preprocess_node_default()
...
function _lean_remove_css($filepath, $styles) {
  $arr_styles = explode("\n",$styles);
...
function lean_preprocess(&$vars, $hook) {
  if($hook == 'page') {
    ...
    if($array_q[0] == 'user'){
      _lean_removetab('Notification settings', $vars);
      _lean_removetab('Track', $vars);
    }
  }
...
function lean_preprocess_node_webform(&$vars) {
  drupal_add_css(path_to_theme() . '/css/node.css', 'theme');
  drupal_add_css(path_to_theme() . '/css/node_webform.css', 'theme');
}

function lean_preprocess_node_homepage(&$vars) {
...
function lean_preprocess_node_page(&$vars) {
  // usual node stuff
  drupal_add_css(path_to_theme() . '/css/node.css', 'theme');
  if ($vars['page']){
...
function lean_preprocess_node_recipe(&$vars) {
  // usual node stuff
  lean_preprocess_node_default($vars);
  if ($vars['page']){
...
function lean_preprocess_node_kanjikanji(&$vars) {
  // usual node stuff
  lean_preprocess_node_default($vars);
...
function lean_preprocess_node_today(&$vars) {
  if (!$vars['is_front']) {
    ...
    if (!$vars['page']) {
      $vars['blog_date'] = _lean_make_blog_date($vars['node']->created, 'node/'.$vars['node']->nid);
      $vars['title'] = _lean_get_today_title($vars['node'], $vars['title'], true);
    } else {
    ...
    // for front page
    $vars['title'] = _lean_get_today_title($vars['node'], $vars['title'], true, false, true);
  }
...
function lean_preprocess_views_view__section_listing(&$vars) {
  if (substr($vars['view']->current_display,0,5)  == 'page_') {
    drupal_add_css(path_to_theme() . '/css/section_index.css', 'theme');
  }
}

function lean_preprocess_views_view__section_listing__page_4(&$vars) {  
  drupal_add_css(path_to_theme() . '/css/section_index.css', 'theme');
  drupal_add_css(path_to_theme() . '/css/node_today.css', 'theme');
...
function lean_preprocess_views_view__topics(&$vars) {  
  if ($vars['view']->current_display == 'page_1') {
    drupal_add_css(path_to_theme() . '/css/section_index.css', 'theme');
  }
  $vars['rows'] = _lean_tag_cloud($vars['view']->result);
}
 ...
 * Preprocess comments
 */
function lean_preprocess_comment(&$vars) {
...
function lean_filter_tips($tips, $long = FALSE, $extra = '') {
  return '';
}
function lean_filter_tips_more_info () {
  return '';
}
...
* @return unknown_type
*/
// Can't name as lean_links() as mothership is using that
